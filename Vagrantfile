...
Vagrant.configure(2) do |config|
	servers.each do |server|
        ...
        # disk1 = './'+server[:release]+'_firstDisk.vdi'
				# unless File.exist?(disk1)
				# 	vb.customize ['createhd', '--filename', disk1, '--variant', 'Fixed', '--size', 10 * 1024]
				# end
				# vb.customize ["storageattach", :id, "--storagectl", "SATA Controller", "--port", 0, "--device", 0, "--type", "hdd", "--medium", disk1 ]	
